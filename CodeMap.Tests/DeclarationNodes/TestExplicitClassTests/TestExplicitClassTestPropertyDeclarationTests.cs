...
{
    public class TestExplicitClassTestPropertyDeclarationTests : DeclarationNodeTests<PropertyDeclaration>, IPropertyDeclarationTests
    {
        protected override bool DeclarationNodePredicate(PropertyDeclaration propertyDeclaration)
            => propertyDeclaration.Name == typeof(ITestExplicitInterface).Namespace + "." + nameof(ITestExplicitInterface) + "." + nameof(ITestExplicitInterface.TestProperty) && propertyDeclaration.DeclaringType.Name == nameof(TestExplicitClass);

        [Fact]
        public void MemberEqualityComparison()
        {
            var propertyInfo = typeof(TestExplicitClass).GetProperty(typeof(ITestExplicitInterface).Namespace + "." + nameof(ITestExplicitInterface) + "." + nameof(ITestExplicitInterface.TestProperty), BindingFlags.NonPublic | BindingFlags.Instance);
        ...
        public void HasNameSet()
            => Assert.Equal("CodeMap.Tests.Data.ITestExplicitInterface.TestProperty", DeclarationNode.Name);
