...
{
    public class TestExplicitClassTestEventDeclarationTests : DeclarationNodeTests<EventDeclaration>, IEventDeclarationTests
    {
        protected override bool DeclarationNodePredicate(EventDeclaration eventDeclaration)
            => eventDeclaration.Name == typeof(ITestExplicitInterface).Namespace + "." + nameof(ITestExplicitInterface) + "." + nameof(ITestExplicitInterface.TestEvent) && eventDeclaration.DeclaringType.Name == nameof(TestExplicitClass);

        [Fact]
        public void MemberEqualityComparison()
        {
            var eventInfo = typeof(TestExplicitClass).GetEvent(typeof(TestExplicitClass).Namespace + "." + nameof(ITestExplicitInterface) + "." + nameof(ITestExplicitInterface.TestEvent), BindingFlags.NonPublic | BindingFlags.Instance);
        ...
        public void HasNameSet()
            => Assert.Equal("CodeMap.Tests.Data.ITestExplicitInterface.TestEvent", DeclarationNode.Name);
