...
{
    public class TestBaseClassClassShadowedTestEventDeclarationTests : DeclarationNodeTests<EventDeclaration>, IEventDeclarationTests
    {
        protected override bool DeclarationNodePredicate(EventDeclaration eventDeclaration)
            => eventDeclaration.Name == nameof(TestBaseClass.ClassShadowedTestEvent) && eventDeclaration.DeclaringType.Name == nameof(TestBaseClass);

        [Fact]
        public void MemberEqualityComparison()
        {
            var eventInfo = typeof(TestBaseClass).GetRuntimeEvent(nameof(TestBaseClass.ClassShadowedTestEvent));
        ...
        public void HasNameSet()
            => Assert.Equal("ClassShadowedTestEvent", DeclarationNode.Name);
