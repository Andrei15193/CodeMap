...
{
    public class TestBaseClassVirtualTestEventDeclarationTests : DeclarationNodeTests<EventDeclaration>, IEventDeclarationTests
    {
        protected override bool DeclarationNodePredicate(EventDeclaration eventDeclaration)
            => eventDeclaration.Name == nameof(TestBaseClass.VirtualTestEvent) && eventDeclaration.DeclaringType.Name == nameof(TestBaseClass);

        [Fact]
        public void MemberEqualityComparison()
        {
            var eventInfo = typeof(TestBaseClass).GetRuntimeEvent(nameof(TestBaseClass.VirtualTestEvent));
