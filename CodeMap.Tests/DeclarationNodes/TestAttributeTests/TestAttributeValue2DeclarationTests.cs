...
{
    public class TestAttributeValue2DeclarationTests : DeclarationNodeTests<PropertyDeclaration>, IPropertyDeclarationTests
    {
        protected override bool DeclarationNodePredicate(PropertyDeclaration propertyDeclaration)
            => propertyDeclaration.Name == nameof(TestAttribute.Value2) && propertyDeclaration.DeclaringType.Name == nameof(TestAttribute);

        [Fact]
        public void MemberEqualityComparison()
        {
            var propertyInfo = typeof(TestAttribute).GetRuntimeProperty(nameof(TestAttribute.Value2));
        ...
        public void HasNameSet()
            => Assert.Equal("Value2", DeclarationNode.Name);
