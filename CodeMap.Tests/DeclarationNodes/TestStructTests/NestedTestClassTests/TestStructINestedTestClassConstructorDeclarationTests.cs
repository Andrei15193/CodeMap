...
{
    public class TestStructINestedTestClassConstructorDeclarationTests : DeclarationNodeTests<ConstructorDeclaration>, IConstructorDeclarationTests
    {
        protected override bool DeclarationNodePredicate(ConstructorDeclaration constructorDeclaration)
            => constructorDeclaration.Name == "NestedTestClass" && constructorDeclaration.DeclaringType.Name == "NestedTestClass" && constructorDeclaration.DeclaringType.DeclaringType.Name == nameof(TestStruct<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var constructorInfo = typeof(TestStruct<>).GetNestedType("NestedTestClass", BindingFlags.NonPublic).GetConstructor(Type.EmptyTypes);
        ...
        public void HasDeclartingTypeSet()
            => Assert.True(typeof(TestStruct<>).GetNestedType("NestedTestClass", BindingFlags.NonPublic) == DeclarationNode.DeclaringType);
