...
{
    public class TestStructInterfaceShadowedTestEventDeclarationTests : DeclarationNodeTests<EventDeclaration>, IEventDeclarationTests
    {
        protected override bool DeclarationNodePredicate(EventDeclaration eventDeclaration)
            => eventDeclaration.Name == typeof(ITestBaseInterface).Namespace + "." + nameof(ITestBaseInterface) + "." + nameof(ITestBaseInterface.InterfaceShadowedTestEvent) && eventDeclaration.DeclaringType.Name == nameof(TestStruct<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var eventInfo = typeof(TestStruct<>).GetEvent(typeof(ITestBaseInterface).Namespace + "." + nameof(ITestBaseInterface) + "." + nameof(ITestBaseInterface.InterfaceShadowedTestEvent), BindingFlags.NonPublic | BindingFlags.Instance);
