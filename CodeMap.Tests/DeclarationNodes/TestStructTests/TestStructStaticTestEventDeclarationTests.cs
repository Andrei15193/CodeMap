...
{
    public class TestStructStaticTestEventDeclarationTests : DeclarationNodeTests<EventDeclaration>, IEventDeclarationTests
    {
        protected override bool DeclarationNodePredicate(EventDeclaration eventDeclaration)
            => eventDeclaration.Name == nameof(TestStruct<int>.StaticTestEvent) && eventDeclaration.DeclaringType.Name == nameof(TestStruct<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var eventInfo = typeof(TestStruct<>).GetRuntimeEvent(nameof(TestStruct<int>.StaticTestEvent));
