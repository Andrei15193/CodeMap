...
{
    public class TestStructToStringDeclarationTests : DeclarationNodeTests<MethodDeclaration>, IMethodDeclarationTests
    {
        protected override bool DeclarationNodePredicate(MethodDeclaration methodDeclaration)
            => methodDeclaration.Name == nameof(TestStruct<int>.ToString) && methodDeclaration.DeclaringType.Name == nameof(TestStruct<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var methodInfo = typeof(TestStruct<>).GetRuntimeMethod(nameof(TestStruct<int>.ToString), Type.EmptyTypes);
        ...
        public void HasNameSet()
            => Assert.Equal("ToString", DeclarationNode.Name);
