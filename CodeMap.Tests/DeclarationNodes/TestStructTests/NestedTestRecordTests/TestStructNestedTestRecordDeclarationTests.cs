...
namespace CodeMap.Tests.DeclarationNodes.TestStructTests.NestedTestRecordTests
{
    public class TestStructNestedTestRecordDeclarationTests : DeclarationNodeTests<RecordDeclaration>, IRecordDeclarationTests
    {
        protected override bool DeclarationNodePredicate(RecordDeclaration recordDeclaration)
            => recordDeclaration.Name == "NestedTestRecord" && recordDeclaration.DeclaringType.Name == nameof(TestStruct<int>);

        [Fact]
        public void TypeEqualityComparison()
        {
            Assert.True(DeclarationNode.Equals(typeof(TestStruct<>).GetNestedType("NestedTestRecord", BindingFlags.NonPublic)));
            Assert.True(DeclarationNode.Equals(typeof(TestStruct<>).GetNestedType("NestedTestRecord", BindingFlags.NonPublic) as object));
            Assert.True(typeof(TestStruct<>).GetNestedType("NestedTestRecord", BindingFlags.NonPublic) == DeclarationNode);
            Assert.True(DeclarationNode == typeof(TestStruct<>).GetNestedType("NestedTestRecord", BindingFlags.NonPublic));
