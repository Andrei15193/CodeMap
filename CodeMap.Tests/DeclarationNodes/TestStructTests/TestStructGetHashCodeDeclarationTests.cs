...
{
    public class TestStructGetHashCodeDeclarationTests : DeclarationNodeTests<MethodDeclaration>, IMethodDeclarationTests
    {
        protected override bool DeclarationNodePredicate(MethodDeclaration methodDeclaration)
            => methodDeclaration.Name == nameof(TestStruct<int>.GetHashCode) && methodDeclaration.DeclaringType.Name == nameof(TestStruct<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var methodInfo = typeof(TestStruct<>).GetRuntimeMethod(nameof(TestStruct<int>.GetHashCode), Type.EmptyTypes);
        ...
        public void HasNameSet()
            => Assert.Equal("GetHashCode", DeclarationNode.Name);
