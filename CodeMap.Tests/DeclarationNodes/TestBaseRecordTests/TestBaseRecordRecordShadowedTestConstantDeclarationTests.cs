...
{
    public class TestBaseRecordRecordShadowedTestConstantDeclarationTests : DeclarationNodeTests<ConstantDeclaration>, IConstantDeclarationTests
    {
        protected override bool DeclarationNodePredicate(ConstantDeclaration constantDeclaration)
            => constantDeclaration.Name == nameof(TestBaseRecord.RecordShadowedTestConstant) && constantDeclaration.DeclaringType.Name == nameof(TestBaseRecord);

        [Fact]
        public void MemberEqualityComparison()
        {
            var fieldInfo = typeof(TestBaseRecord).GetField("RecordShadowedTestConstant", BindingFlags.Public | BindingFlags.Static);
