...
{
    public class TestBaseRecordRecordShadowedTestMethodDeclarationTests : DeclarationNodeTests<MethodDeclaration>, IMethodDeclarationTests
    {
        protected override bool DeclarationNodePredicate(MethodDeclaration methodDeclaration)
            => methodDeclaration.Name == nameof(TestBaseRecord.RecordShadowedTestMethod) && methodDeclaration.DeclaringType.Name == nameof(TestBaseRecord);

        [Fact]
        public void MemberEqualityComparison()
        {
            var methodInfo = typeof(TestBaseRecord).GetRuntimeMethod(nameof(TestBaseRecord.RecordShadowedTestMethod), Type.EmptyTypes);
        ...
        public void HasNameSet()
            => Assert.Equal("RecordShadowedTestMethod", DeclarationNode.Name);
