...
{
    public class TestBaseRecordRecordShadowedTestEventDeclarationTests : DeclarationNodeTests<EventDeclaration>, IEventDeclarationTests
    {
        protected override bool DeclarationNodePredicate(EventDeclaration eventDeclaration)
            => eventDeclaration.Name == nameof(TestBaseRecord.RecordShadowedTestEvent) && eventDeclaration.DeclaringType.Name == nameof(TestBaseRecord);

        [Fact]
        public void MemberEqualityComparison()
        {
            var eventInfo = typeof(TestBaseRecord).GetRuntimeEvent(nameof(TestBaseRecord.RecordShadowedTestEvent));
        ...
        public void HasNameSet()
            => Assert.Equal("RecordShadowedTestEvent", DeclarationNode.Name);
