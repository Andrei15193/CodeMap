...
{
    public class TestBaseRecordAbstractTestEventDeclarationTests : DeclarationNodeTests<EventDeclaration>, IEventDeclarationTests
    {
        protected override bool DeclarationNodePredicate(EventDeclaration eventDeclaration)
            => eventDeclaration.Name == nameof(TestBaseRecord.AbstractTestEvent) && eventDeclaration.DeclaringType.Name == nameof(TestBaseRecord);

        [Fact]
        public void MemberEqualityComparison()
        {
            var eventInfo = typeof(TestBaseRecord).GetRuntimeEvent(nameof(TestBaseRecord.AbstractTestEvent));
