...
{
    public class TestBaseRecordShadowedTestFieldDeclarationTests : DeclarationNodeTests<FieldDeclaration>, IFieldDeclarationTests
    {
        protected override bool DeclarationNodePredicate(FieldDeclaration fieldDeclaration)
            => fieldDeclaration.Name == "ShadowedTestField" && fieldDeclaration.DeclaringType.Name == nameof(TestBaseRecord);

        [Fact]
        public void MemberEqualityComparison()
        {
            var fieldInfo = typeof(TestBaseRecord).GetField("ShadowedTestField", BindingFlags.NonPublic | BindingFlags.Instance);
