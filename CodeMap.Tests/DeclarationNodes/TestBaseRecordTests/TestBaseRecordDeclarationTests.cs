...
{
    public class TestBaseRecordDeclarationTests : DeclarationNodeTests<RecordDeclaration>, IRecordDeclarationTests
    {
        protected override bool DeclarationNodePredicate(RecordDeclaration recordDeclaration)
            => recordDeclaration.Name == nameof(TestBaseRecord);

        [Fact]
        public void TypeEqualityComparison()
        {
            Assert.True(DeclarationNode.Equals(typeof(TestBaseRecord)));
            Assert.True(DeclarationNode.Equals(typeof(TestBaseRecord) as object));
            Assert.True(typeof(TestBaseRecord) == DeclarationNode);
            Assert.True(DeclarationNode == typeof(TestBaseRecord));
        ...
        public void HasPropertiesSet()
            => Assert.Equal(5, DeclarationNode.Properties.Count);
