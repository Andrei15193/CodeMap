...
{
    public class TestClassTestMethod23DeclarationTests : DeclarationNodeTests<MethodDeclaration>, IMethodDeclarationTests
    {
        protected override bool DeclarationNodePredicate(MethodDeclaration methodDeclaration)
            => methodDeclaration.Name == nameof(TestClass<int>.TestMethod23) && methodDeclaration.DeclaringType.Name == nameof(TestClass<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var methodInfo = typeof(TestClass<>).GetRuntimeMethod(nameof(TestClass<int>.TestMethod23), new[] { typeof(TestClass<>).GetGenericArguments().Single().MakeByRefType() });
        ...
        public void HasNameSet()
            => Assert.Equal("TestMethod23", DeclarationNode.Name);
