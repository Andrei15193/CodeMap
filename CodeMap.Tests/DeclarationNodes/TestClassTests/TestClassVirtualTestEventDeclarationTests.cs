...
{
    public class TestClassVirtualTestEventDeclarationTests : DeclarationNodeTests<EventDeclaration>, IEventDeclarationTests
    {
        protected override bool DeclarationNodePredicate(EventDeclaration eventDeclaration)
            => eventDeclaration.Name == nameof(TestClass<int>.VirtualTestEvent) && eventDeclaration.DeclaringType.Name == nameof(TestClass<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var eventInfo = typeof(TestClass<>).GetRuntimeEvent(nameof(TestClass<int>.VirtualTestEvent));
