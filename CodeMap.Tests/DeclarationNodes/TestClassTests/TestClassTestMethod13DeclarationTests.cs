...
{
    public class TestClassTestMethod13DeclarationTests : DeclarationNodeTests<MethodDeclaration>, IMethodDeclarationTests
    {
        protected override bool DeclarationNodePredicate(MethodDeclaration methodDeclaration)
            => methodDeclaration.Name == nameof(TestClass<int>.TestMethod13) && methodDeclaration.DeclaringType.Name == nameof(TestClass<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var methodInfo = typeof(TestClass<>).GetRuntimeMethod(nameof(TestClass<int>.TestMethod13), new[] { typeof(TestClass<int>.NestedTestClass<byte[], IEnumerable<string>>) });
        ...
        public void HasNameSet()
            => Assert.Equal("TestMethod13", DeclarationNode.Name);
