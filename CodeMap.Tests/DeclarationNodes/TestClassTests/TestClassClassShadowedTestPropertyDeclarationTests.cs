...
{
    public class TestClassClassShadowedTestPropertyDeclarationTests : DeclarationNodeTests<PropertyDeclaration>, IPropertyDeclarationTests
    {
        protected override bool DeclarationNodePredicate(PropertyDeclaration propertyDeclaration)
            => propertyDeclaration.Name == nameof(TestClass<int>.ClassShadowedTestProperty) && propertyDeclaration.DeclaringType.Name == nameof(TestClass<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var propertyInfo = typeof(TestClass<>).GetProperty(nameof(TestClass<int>.ClassShadowedTestProperty), BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly);
        ...
        public void HasNameSet()
            => Assert.Equal("ClassShadowedTestProperty", DeclarationNode.Name);
