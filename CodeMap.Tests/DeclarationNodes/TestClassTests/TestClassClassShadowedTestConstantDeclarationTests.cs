...
{
    public class TestClassClassShadowedTestConstantDeclarationTests : DeclarationNodeTests<ConstantDeclaration>, IConstantDeclarationTests
    {
        protected override bool DeclarationNodePredicate(ConstantDeclaration constantDeclaration)
            => constantDeclaration.Name == nameof(TestClass<int>.ClassShadowedTestConstant) && constantDeclaration.DeclaringType.Name == nameof(TestClass<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var fieldInfo = typeof(TestClass<>).GetField("ClassShadowedTestConstant", BindingFlags.Public | BindingFlags.Static);
