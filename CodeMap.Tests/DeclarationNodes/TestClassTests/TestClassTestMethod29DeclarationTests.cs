...
{
    public class TestClassTestMethod29DeclarationTests : DeclarationNodeTests<MethodDeclaration>, IMethodDeclarationTests
    {
        protected override bool DeclarationNodePredicate(MethodDeclaration methodDeclaration)
            => methodDeclaration.Name == nameof(TestClass<int>.TestMethod29) && methodDeclaration.DeclaringType.Name == nameof(TestClass<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var methodInfo = typeof(TestClass<>).GetRuntimeMethod(nameof(TestClass<int>.TestMethod29), new[] { typeof(decimal*[]).MakeByRefType() });
        ...
        public void HasNameSet()
            => Assert.Equal("TestMethod29", DeclarationNode.Name);
