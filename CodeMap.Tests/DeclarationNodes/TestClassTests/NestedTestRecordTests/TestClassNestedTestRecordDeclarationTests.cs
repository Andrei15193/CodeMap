...
namespace CodeMap.Tests.DeclarationNodes.TestClassTests.NestedTestRecordTests
{
    public class TestClassNestedTestRecordDeclarationTests : DeclarationNodeTests<RecordDeclaration>, IRecordDeclarationTests
    {
        protected override bool DeclarationNodePredicate(RecordDeclaration recordDeclaration)
            => recordDeclaration.Name == nameof(TestClass<int>.NestedTestRecord) && recordDeclaration.DeclaringType.Name == nameof(TestClass<int>);

        [Fact]
        public void TypeEqualityComparison()
        {
            Assert.True(DeclarationNode.Equals(typeof(TestClass<>.NestedTestRecord)));
            Assert.True(DeclarationNode.Equals(typeof(TestClass<>.NestedTestRecord) as object));
            Assert.True(typeof(TestClass<>.NestedTestRecord) == DeclarationNode);
            Assert.True(DeclarationNode == typeof(TestClass<>.NestedTestRecord));
