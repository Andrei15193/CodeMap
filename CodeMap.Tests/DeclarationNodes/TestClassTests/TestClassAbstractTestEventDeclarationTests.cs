...
{
    public class TestClassAbstractTestEventDeclarationTests : DeclarationNodeTests<EventDeclaration>, IEventDeclarationTests
    {
        protected override bool DeclarationNodePredicate(EventDeclaration eventDeclaration)
            => eventDeclaration.Name == nameof(TestClass<int>.AbstractTestEvent) && eventDeclaration.DeclaringType.Name == nameof(TestClass<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var eventInfo = typeof(TestClass<>).GetRuntimeEvent(nameof(TestClass<int>.AbstractTestEvent));
