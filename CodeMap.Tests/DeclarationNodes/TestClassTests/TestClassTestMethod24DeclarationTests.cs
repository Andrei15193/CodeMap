...
{
    public class TestClassTestMethod24DeclarationTests : DeclarationNodeTests<MethodDeclaration>, IMethodDeclarationTests
    {
        protected override bool DeclarationNodePredicate(MethodDeclaration methodDeclaration)
            => methodDeclaration.Name == nameof(TestClass<int>.TestMethod24) && methodDeclaration.DeclaringType.Name == nameof(TestClass<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var methodInfo = typeof(TestClass<>).GetRuntimeMethod(nameof(TestClass<int>.TestMethod24), new[] { typeof(TestClass<>).GetGenericArguments().Single().MakeByRefType() });
        ...
        public void HasNameSet()
            => Assert.Equal("TestMethod24", DeclarationNode.Name);
