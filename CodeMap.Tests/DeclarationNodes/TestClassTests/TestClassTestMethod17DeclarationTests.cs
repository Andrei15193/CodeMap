...
{
    public class TestClassTestMethod17DeclarationTests : DeclarationNodeTests<MethodDeclaration>, IMethodDeclarationTests
    {
        protected override bool DeclarationNodePredicate(MethodDeclaration methodDeclaration)
            => methodDeclaration.Name == nameof(TestClass<int>.TestMethod17) && methodDeclaration.DeclaringType.Name == nameof(TestClass<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var methodInfo = typeof(TestClass<>).GetRuntimeMethod(nameof(TestClass<int>.TestMethod17), new[] { typeof(TestClass<int>.NestedTestClass<byte[], IEnumerable<string>>[]).MakeByRefType() });
        ...
        public void HasNameSet()
            => Assert.Equal("TestMethod17", DeclarationNode.Name);
