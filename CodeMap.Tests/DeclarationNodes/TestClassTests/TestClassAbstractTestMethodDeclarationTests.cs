...
{
    public class TestClassAbstractTestMethodDeclarationTests : DeclarationNodeTests<MethodDeclaration>, IMethodDeclarationTests
    {
        protected override bool DeclarationNodePredicate(MethodDeclaration methodDeclaration)
            => methodDeclaration.Name == nameof(TestClass<int>.AbstractTestMethod) && methodDeclaration.DeclaringType.Name == nameof(TestClass<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var methodInfo = typeof(TestClass<>).GetRuntimeMethod(nameof(TestClass<int>.AbstractTestMethod), Type.EmptyTypes);
