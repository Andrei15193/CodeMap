...
{
    public class TestClassTestMethod15DeclarationTests : DeclarationNodeTests<MethodDeclaration>, IMethodDeclarationTests
    {
        protected override bool DeclarationNodePredicate(MethodDeclaration methodDeclaration)
            => methodDeclaration.Name == nameof(TestClass<int>.TestMethod15) && methodDeclaration.DeclaringType.Name == nameof(TestClass<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var methodInfo = typeof(TestClass<>).GetRuntimeMethod(nameof(TestClass<int>.TestMethod15), new[] { typeof(TestClass<int>.NestedTestClass<byte[], IEnumerable<string>>).MakeByRefType() });
        ...
        public void HasNameSet()
            => Assert.Equal("TestMethod15", DeclarationNode.Name);
