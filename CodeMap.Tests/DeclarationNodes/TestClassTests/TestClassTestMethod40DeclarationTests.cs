...
{
    public class TestClassTestMethod40DeclarationTests : DeclarationNodeTests<MethodDeclaration>, IMethodDeclarationTests
    {
        protected override bool DeclarationNodePredicate(MethodDeclaration methodDeclaration)
            => methodDeclaration.Name == nameof(TestClass<int>.TestMethod40) && methodDeclaration.DeclaringType.Name == nameof(TestClass<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var methodInfo = typeof(TestClass<>).GetRuntimeMethod(nameof(TestClass<int>.TestMethod40), new[] { Type.MakeGenericMethodParameter(0).MakeByRefType() });
        ...
        public void HasNameSet()
            => Assert.Equal("TestMethod40", DeclarationNode.Name);
