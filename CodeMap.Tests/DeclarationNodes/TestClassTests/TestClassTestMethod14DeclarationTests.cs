...
{
    public class TestClassTestMethod14DeclarationTests : DeclarationNodeTests<MethodDeclaration>, IMethodDeclarationTests
    {
        protected override bool DeclarationNodePredicate(MethodDeclaration methodDeclaration)
            => methodDeclaration.Name == nameof(TestClass<int>.TestMethod14) && methodDeclaration.DeclaringType.Name == nameof(TestClass<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var methodInfo = typeof(TestClass<>).GetRuntimeMethod(nameof(TestClass<int>.TestMethod14), new[] { typeof(TestClass<int>.NestedTestClass<byte[], IEnumerable<string>>[]) });
        ...
        public void HasNameSet()
            => Assert.Equal("TestMethod14", DeclarationNode.Name);
