...
{
    public class TestRecordVirtualTestEventDeclarationTests : DeclarationNodeTests<EventDeclaration>, IEventDeclarationTests
    {
        protected override bool DeclarationNodePredicate(EventDeclaration eventDeclaration)
            => eventDeclaration.Name == nameof(TestRecord<int>.VirtualTestEvent) && eventDeclaration.DeclaringType.Name == nameof(TestRecord<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var eventInfo = typeof(TestRecord<>).GetRuntimeEvent(nameof(TestRecord<int>.VirtualTestEvent));
        ...
        public void HasNameSet()
            => Assert.Equal("VirtualTestEvent", DeclarationNode.Name);
