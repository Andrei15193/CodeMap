...
{
    public class TestRecordNestedTestRecordDeclarationTests : DeclarationNodeTests<RecordDeclaration>, IRecordDeclarationTests
    {
        protected override bool DeclarationNodePredicate(RecordDeclaration recordDeclaration)
            => recordDeclaration.Name == nameof(TestRecord<int>.NestedTestRecord) && recordDeclaration.DeclaringType.Name == nameof(TestRecord<int>);

        [Fact]
        public void TypeEqualityComparison()
        {
            Assert.True(DeclarationNode.Equals(typeof(TestRecord<>.NestedTestRecord)));
            Assert.True(DeclarationNode.Equals(typeof(TestRecord<>.NestedTestRecord) as object));
            Assert.True(typeof(TestRecord<>.NestedTestRecord) == DeclarationNode);
            Assert.True(DeclarationNode == typeof(TestRecord<>.NestedTestRecord));
