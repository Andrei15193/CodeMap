...
{
    public class TestRecordAbstractTestMethodDeclarationTests : DeclarationNodeTests<MethodDeclaration>, IMethodDeclarationTests
    {
        protected override bool DeclarationNodePredicate(MethodDeclaration methodDeclaration)
            => methodDeclaration.Name == nameof(TestRecord<int>.AbstractTestMethod) && methodDeclaration.DeclaringType.Name == nameof(TestRecord<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var methodInfo = typeof(TestRecord<>).GetRuntimeMethod(nameof(TestRecord<int>.AbstractTestMethod), Type.EmptyTypes);
        ...
        public void HasNameSet()
            => Assert.Equal("AbstractTestMethod", DeclarationNode.Name);
        ...
        public void HasIsOverrideSet()
            => Assert.True(DeclarationNode.IsOverride);
        ...
        public void HasReturnTypeSet()
            => Assert.True(typeof(string) == DeclarationNode.Return.Type);
