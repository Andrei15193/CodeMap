...
{
    public class TestRecordAbstractTestEventDeclarationTests : DeclarationNodeTests<EventDeclaration>, IEventDeclarationTests
    {
        protected override bool DeclarationNodePredicate(EventDeclaration eventDeclaration)
            => eventDeclaration.Name == nameof(TestRecord<int>.AbstractTestEvent) && eventDeclaration.DeclaringType.Name == nameof(TestRecord<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var eventInfo = typeof(TestRecord<>).GetRuntimeEvent(nameof(TestRecord<int>.AbstractTestEvent));
