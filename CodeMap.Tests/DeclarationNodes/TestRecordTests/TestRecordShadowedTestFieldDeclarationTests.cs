...
{
    public class TestRecordShadowedTestFieldDeclarationTests : DeclarationNodeTests<FieldDeclaration>, IFieldDeclarationTests
    {
        protected override bool DeclarationNodePredicate(FieldDeclaration fieldDeclaration)
            => fieldDeclaration.Name == "ShadowedTestField" && fieldDeclaration.DeclaringType.Name == nameof(TestRecord<int>);

        [Fact]
        public void MemberEqualityComparison()
        {
            var fieldInfo = typeof(TestRecord<>).GetField("ShadowedTestField", BindingFlags.NonPublic | BindingFlags.Instance);
