...
namespace CodeMap.ElementsTests
{
    ...
    public class DocumentationElementTests
    {
        ...
        [Fact]
        public void CreatingReturnsElementWithNullTypeThrowsException()
        {
            var exception = Assert.Throws<ArgumentNullException>(
                "typeReferenceData",
                () => DocumentationElement.Returns(
                    null,
                    Enumerable.Empty<BlockDocumentationElement>()
                )
            );

            Assert.Equal(new ArgumentNullException("typeReferenceData").Message, exception.Message);
        }

        [Fact]
        public void CreatingReturnsElementWithNullContentThrowsException()
        {
            var exception = Assert.Throws<ArgumentNullException>(
                "content",
                () => DocumentationElement.Returns(
                    _GetTypeReferenceFor(typeof(string)),
                    null
                )
            );
        ...
        [Fact]
        public void CreatingReturnsElementWithContentContainingNullThrowsException()
        {
            var exception = Assert.Throws<ArgumentException>(
                "content",
                () => DocumentationElement.Returns(
                    _GetTypeReferenceFor(typeof(string)),
                    new BlockDocumentationElement[] { null }
                )
            );
        ...
        [Fact]
        public void CreatingReturnsElementWithVoidReturnTypeThrowsException()
        {
            var exception = Assert.Throws<ArgumentException>(
                "typeReferenceData",
                () => DocumentationElement.Returns(
            ...
            Assert.Equal(new ArgumentException("Cannot be 'void' return type.", "typeReferenceData").Message, exception.Message);
        }

        [Fact]
        public async Task ReturnsElementCallsBeginningAndEndingVisitorMethods()
        {
            var typeReference = _GetTypeReferenceFor(typeof(string));
            var visitorMock = new Mock<IDocumentationVisitor>();
            var returns = DocumentationElement.Returns(typeReference, Enumerable.Empty<BlockDocumentationElement>());

            await visitorMock.VerifyAcceptMethods(
                returns,
                visitor => visitor.VisitReturnsBeginning(typeReference),
                visitor => visitor.VisitReturnsEnding()
            );
        }

        [Fact]
        public async Task ReturnsElementCallsVisitorMethods()
        {
            const string code = "piece of code";
            var typeReference = _GetTypeReferenceFor(typeof(string));

            var visitorMock = new Mock<IDocumentationVisitor>();
            var returns = DocumentationElement.Returns(
                typeReference,
                new BlockDocumentationElement[]
                {
                    DocumentationElement.Paragraph(Enumerable.Empty<InlineDocumentationElement>()),
        ...
        private ExternalTypeReferenceData _GetTypeReferenceFor(Type type)
            => new ExternalTypeReferenceData(
                type.Name,
                type.Namespace,
                null,
                _GetAssemblyReference(type.Assembly.GetName())
            );

        private AssemblyReferenceData _GetAssemblyReference(AssemblyName assemblyName)
            => new AssemblyReferenceData(
                assemblyName.Name,
                assemblyName.Version,
                assemblyName.CultureName,
                string.Join(string.Empty, assemblyName.GetPublicKeyToken().Select(@byte => @byte.ToString("X2")))
            );
    }
}