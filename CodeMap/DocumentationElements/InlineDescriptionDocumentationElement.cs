...
{
    /// <summary>Represents a collection of <see cref="InlineDocumentationElement"/> contained by an XML element.</summary>
    public class InlineDescriptionDocumentationElement : DocumentationElement, IReadOnlyList<InlineDocumentationElement>
    {
        private readonly IReadOnlyList<InlineDocumentationElement> _inlineElements;

        internal InlineDescriptionDocumentationElement(IEnumerable<InlineDocumentationElement> inlineElements, IReadOnlyDictionary<string, string> xmlAttributes)
        {
            _inlineElements = inlineElements
                .AsReadOnlyList()
                ?? throw new ArgumentNullException(nameof(inlineElements));
            if (_inlineElements.Contains(null))
                throw new ArgumentException("Cannot contain 'null' elements.", nameof(inlineElements));
        ...
        public InlineDocumentationElement this[int index]
        {
            get
            {
                try
                {
                    return _inlineElements[index];
                }
        ...
        public IEnumerator<InlineDocumentationElement> GetEnumerator()
            => _inlineElements.GetEnumerator();
